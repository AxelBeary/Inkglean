...
// 波12：首次启动引导守卫（未完成先进引导页，引导页无需登录态）。
import { createRouter, createWebHashHistory } from 'vue-router'
import { useAuthStore } from '../stores/auth'
import { isOnboarded } from '../tools/onboarding'
...
router.beforeEach(async (to) => {
  ...
  // 首次启动引导（波12）：未完成先进引导页（全可跳过，标记后不再拦）
  if (!isOnboarded() && to.name !== 'onboarding') return { name: 'onboarding' }
