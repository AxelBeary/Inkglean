...
## 看板（2026-09-13 第五次刷新：定时开工 P1~P5 五批已落地，🔴 未提交）

- **HEAD** `d4984edf`（已推送的那笔）；**工作区挂着五批成果未提交**，故本行 HEAD 与待提交内容不同步
- **基线**：server **1801**（156 文件）/ web **876**（124）/ desktop **447**（30）/ shared **23**（4）/ E2E **13**（本轮未复跑）；迁移 **v76**；版本 server·web 1.0.1、桌面 0.1.0
- 🔴 **十七道门禁未跑**：`scripts/accept.ps1:56` 前置检查在脏工作区直接 `exit 2`，必须先提交才能验收 → 本轮只称「四端各端门禁实测全绿」，**不称「门禁全绿」**
...
1. **等用户下 commit 令** → 按领地逐文件提交（禁 `git add -A`）→ 跑 `pwsh scripts/accept.ps1` 十七道 → 若红补修
...
> - 🔴 **未过十七道**：脏工作区被 `accept.ps1` 前置检查拦下（038 事故防线，设计如此）→ 待提交后补跑。计划书 §四/§五 把「先跑绿」排在「后提交」之前，与脚本前置互斥，**实操顺序必须反过来**，已登记。
