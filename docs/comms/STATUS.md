...
## 看板（2026-09-12 第四次刷新：五笔提交已推送，CI 四 job 全绿）

- **HEAD** `81e49576` **已推送**，**CI 四 job（server / web / docker / desktop）+ E2E 全绿**；工作区干净
- 🔴 **推送后唯一一次红是主代理自己造成的**：给 `desktop/src-tauri/tauri.conf.json` 加版权字段时顺手写了 `//` 注释，而 `tauri-build` 用**严格 JSON 解析器**→ CI 只有 desktop job（cargo check）红；已删注释修正（`81e49576`）。教训：**改 `desktop/src-tauri/` 必须本地跑 `cargo check`**——上一笔只跑了 server/web 就交，本地“全绿”是假全绿
- **基线**：server **1715** / web **869**（+8 拆后专项）/ desktop **281** / shared **23** / E2E **13**；迁移 **v74**；版本 server·web 1.0.1、桌面 0.1.0
- **默认窗 1200×820**（9/5 拍板；最小窗 1200×600 不动＝826 亲定）
- **无在途施工**；公网冻结中（830 审计批与 9/3 依赖批仍在公网之外）
- ✅ **门禁缺口已清**：巨型文件防阀 10 项超线**全部拆完归零**，`check-file-size.mjs` 的 ALLOWLIST **清空为 0 项**（规则同时收紧，见 F-09 条）；accept.ps1 17 道
- 🔴 **上线前置硬警告（本批新增）**：管理员判定已改为只认库值，公网部署前**必须先确认 `platform_config.admin_qq` 非空**，否则升级后无人能进后台（详见本页 9/12 条）

**下一步（按优先级）**

1. **🔴 冒烟测试工单 S-912（待做，用户已指定由新会话执行）** — 全单在交接档第 5 节：A 组 18 条登录后逐页走查（对应本批拆的 11 个组件）/ B 组开箱向导第 3 步（需空库实例，禁动现有 commission.db）/ C 组问卷真机与桌面波1 活体 12 条 / D 组字号最大与暗色与窄窗与空态。本地容器已重建至最新代码，入口 `http://localhost:3000`
2. **问卷真机过目** — `docs/requirements/REQ-044-附件-问卷.html` 需用户手机 QQ 实点（长按复制、软键盘顶页两项自动化测不了），过了才能发群
3. **开箱向导新步的真后端联调** — 本批只用 mock 服务器验过（e2e 无向导覆盖），上线前需真机走一遍五步
