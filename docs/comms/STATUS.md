...
- **HEAD** `a6ac12a9`（⏳ 待推送：N2 门禁工具批 + F-44 防阀扩射与 TplGallery 拆分；其前 `fff6a8a7` 已同步 origin/master）
- **基线（9/20 直跑实测，全 exit 0）**：server **1847**（159 文件）/ web **959**（140）/ desktop **538**（37）/ shared **27**（4）/ E2E **14**（33.5s）
  - 迁移 **v76**；版本 server·web 1.0.1、桌面 0.1.0；`scripts/accept-baseline.json` 已同步至此实测值（此前滞后停在 1801/886/447/23）
...
2. **N2 批推送收尾**：`a6ac12a9` 已提交，门禁全绿后推送；看板基线数字对应 9/20 工作区实跑态，并行批合入后需再同步
...
> 📌 **2026-09-25 「今天吃什么」库扩建批（两波采集 + 终审波）：495 → 3530 条，单文件拆为 `web/src/utils/food-menu/` 13 分片（未提交）**
>
...
> - **待用户**：本批未提交；草稿、十二份验证/终审报告与可复跑脚本（merge/merge2/merge3/fam）在 `temp/food-expand-20260925/`（gitignored）可查逐条裁决；结构操作前快照存该目录 backup-pre-merge3/。
