...
2. **单行不超 200 字**：新条目写成「短标题行 + `- ` 分组要点」，**禁止把整批结论塞进一个长行**。三条理由均实测踩过：超长行会让 ripgrep 漏报（假阴性）、让 diff 不可读（等于没防线）、让精确替换易错。已挂机械防阀：`node scripts/check-status-line.mjs`（只判本次变更新增行，存量长行不报；已进 accept.ps1 门禁，见下）。
